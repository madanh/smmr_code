...
the dataset filenames and run sample.py. The trace is then saved with in an .npy
...
Please set `enable_logging` to False if you want to debug ths file.

Use modesplit.py to make reproduce separate mode analysis in the article (again
some plotting functionality pending).
Please set enable_logging to False if you want to debug ths file.

Use 'lse_from_gen.py' to generate least squares estimates. Set `truth_source` to
